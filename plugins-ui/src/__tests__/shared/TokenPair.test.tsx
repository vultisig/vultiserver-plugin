...
describe("TokenPair Component", () => {
  it("should render known token names", () => {
    ...
    screen.getByText("USDC", { exact: true });
    screen.getByText("WETH", { exact: true });
  });

  it("should render unknown token when token is not in the supported list", () => {
    render(<TokenPair data={["token_1", "token_2"]} />);

    screen.getByText("Unknown token address: token_1", { exact: true });
    screen.getByText("Unknown token address: token_2", { exact: true });
  });
});
