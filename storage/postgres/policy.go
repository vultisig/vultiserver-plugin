...
func (p *PostgresBackend) GetAllPluginPolicies(ctx context.Context, publicKey string, pluginType string, take int, skip int) (types.PluginPolicyDto, error) {
	if p.pool == nil {
		return types.PluginPolicyDto{}, fmt.Errorf("database pool is nil")
	}
	...
	if err != nil {
		return types.PluginPolicyDto{}, err
	}
	defer rows.Close()
